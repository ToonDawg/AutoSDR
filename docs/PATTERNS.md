...
| Outbound HTTP        | httpx                                            | Async-first, FastAPI-aligned.                      | `autosdr/connectors/` and `autosdr/enrichment.py` | requests, urllib in app code                                             | LLM HTTP is owned by LiteLLM; do not call providers via httpx directly. Per-host fetches for lead enrichment (homepage / robots.txt / sitemap) live in `autosdr/enrichment.py` — see ticket 0011. Lifetimes: the shared `httpx.AsyncClient` is owned by the FastAPI lifespan in `autosdr/webhook.py` and threaded through `autosdr/scheduler.py` / `autosdr/pipeline/outreach.py`. |
