...
| PWA shell + SW        | `vite-plugin-pwa` with `injectManifest` strategy | Plugin handles precache + manifest emit; we own a tiny SW (`src/sw/sw.ts`) with bespoke `push` + `notificationclick` handlers. | All PWA features                          | Hand-rolled service worker registration, separate Workbox CLI, `next-pwa`-style wrappers | SW has its own `tsconfig.sw.json` (WebWorker lib only). Registration helper: `src/sw/register.ts` (no-op in dev). See ticket 0005. |
...
| Web Push transport   | `pywebpush` wrapped in `asyncio.to_thread`        | One library encapsulates VAPID + payload encryption; thread-pool exemption from the httpx rule because `pywebpush` is sync-by-design. | `autosdr/push.py`                         | Calling `requests` directly to push gateways, hand-rolling Web Push encryption | HTTP 404 / 410 from the gateway hard-deletes the row; other failures stamp `last_error`. Privacy posture pinned by `tests/test_push_payload_privacy.py`. See ticket 0005. |
| Crypto primitives    | `cryptography` (P-256 keypair gen for VAPID)     | Standard PyCA library; only used for keygen at first boot. | `autosdr/push.py` (VAPID keygen only)     | Hand-rolling EC keypair encoding, shelling out to `openssl`              | Key bytes never leave `autosdr.push.ensure_vapid_keys`. Persisted base64url on `workspace.settings.push.vapid_*`. |
...
| 2026-05-02 | `vite-plugin-pwa` (injectManifest) blessed for PWA shell + SW. | Plugin handles precache + manifest; we own ~80 LoC SW with bespoke push handlers. SW has its own `tsconfig.sw.json`. See ticket 0005. |
| 2026-05-02 | `pywebpush` blessed for Web Push transport, `cryptography` for VAPID keygen. | `pywebpush` is the single Python Web Push library; uses `asyncio.to_thread` from the HITL hot-path. `cryptography` is the only other crypto dep — bounded to one helper. |
