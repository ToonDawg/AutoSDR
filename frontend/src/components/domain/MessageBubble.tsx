...
export function MessageBubble({ message, leadName }: { message: Message; leadName: string }) {
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-mono uppercase tracking-[0.14em] text-ink-faint">
        {typeof message.metadata.eval_score === 'number' && (
          <span>
            eval <span className="text-ink-muted">{Math.round(message.metadata.eval_score * 100)}</span>
          </span>
        )}
        {message.metadata.attempt_count != null && message.metadata.attempt_count > 1 && (
          <span>
            attempts <span className="text-ink-muted">{message.metadata.attempt_count}</span>
          </span>
        )}
        {message.metadata.prompt_version && (
          <span>
            prompt <span className="text-ink-muted">{message.metadata.prompt_version}</span>
          </span>
        )}
        {message.metadata.model && (
          <span>
            model <span className="text-ink-muted normal-case">{message.metadata.model}</span>
          </span>
        )}
        {message.metadata.intent && (
          <span>
            intent <span className="text-rust">{INTENT_LABEL[message.metadata.intent]}</span>
            {message.metadata.confidence != null && (
              <span className="text-ink-muted"> · {Math.round(message.metadata.confidence * 100)}%</span>
            )}
          </span>
        )}
      </div>
